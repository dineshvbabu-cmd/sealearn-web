...
export default async function EditPackagePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const selectedIds = new Set(pkg.courses.map((c) => c.courseId));
  ...
  return (
    <>
      <div className="mb-6 flex items-center gap-3">
        ...
        <div>
          <h1 className="font-cinzel text-2xl font-bold text-navy">Edit Package</h1>
          <p className="text-muted text-sm mt-0.5">{pkg.title}</p>
        </div>
      </div>
      ...
      <div className="max-w-3xl">
        <form action={action} className="space-y-6">
          ...
          <div className="bg-white rounded-xl border border-border shadow-sm p-6">
            <h2 className="font-bold text-navy mb-1 pb-2 border-b border-border">Courses in this Bundle</h2>
            <p className="text-muted text-xs mb-4">Tick courses to include. Untick to remove.</p>
              ...
              {courses.map((c) => (
                <label key={c.id} className="flex items-center gap-3 p-3 rounded-lg border border-border hover:bg-surface cursor-pointer transition-colors">
                  <input type="checkbox" name="courseIds" value={c.id}
                    defaultChecked={selectedIds.has(c.id)} className="accent-teal w-4 h-4" />
